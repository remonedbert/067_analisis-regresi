...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\bagan\\Downloads\\Susenas Anreg.xlsx")
dt
```

```{r}
y <- dt$"Rata Rata Pengeluaran makanan sebulan"
x <- dt$"Rata-rata Pengeluaran Perkapita Sebulan"
n <- length(dt)
```
...
$$E[\hat{Y}] = \hat{Y} = (1.890e+06) + (6.065e-01)X$$
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{0.2117} \approx 0.46$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 1.890e+06   dan 6.065e-01. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah  [-129039.9 ,  3909047] dan untuk parameter $β_1$ adalah [ 0.4447305 ,  0.7681785 ]
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=10 adalah 1890010 . Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang 1498047 hingga 2281973.
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah 1890010. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang  -2165370 hingga 5945390.